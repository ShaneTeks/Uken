...
// NOTE: Changed to 'location-images' to use an existing bucket from your Supabase setup.
// For better organization, consider creating a dedicated bucket like 'shot_images' or 'project_images'.
// Ensure this bucket exists in your Supabase project and has appropriate public read access policies.
...
const CreatePage = (): JSX.Element => {
  ...
  // Memoized selectors (operate on client-side state)
  ...
  // --- Navigation Handlers (Client-side) ---
  ...
  const handleStartEdit = (item: EditableNamedItem, type: EditableItemType) => {
    ...
    setEditingDescriptionItem(null); // Close description editing if open
  };
  ...
  const handleStartEditDescription = (item: Chapter | Episode | Scene, type: 'chapter' | 'episode' | 'scene') => {
    ...
    setEditingItem(null); // Close name editing if open
  };
  ...
  const handleSaveDescription = async () => {
    ...
    try {
      const { error: updateError } = await supabase
        .from(type + 's') // chapters, episodes, scenes
          ...
          scenes: e.scenes.map(sc => {
            if (sc.id === sceneId) {
              if (shotIdToUpdate) {
                ...
                // This case should ideally be handled by ensuring a shot always has an ID before this function is called
                // If adding a new shot, ensure it's given a temporary ID before calling this.
                const newShotBase: Shot = {
                    id: `temp-${Date.now()}`, // Fallback temporary ID
                    name: 'New Shot',
                    scene_id: sceneId,
                    isAnimating: false,
                    isGenerating: false,
  ...
  const handleFileUploadShot = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    const filePath = `public/${selectedScene.id}/${fileName}`; // Path for image uploads
    ...
    try {
      ...
      if (existingShot && existingShot.id && !existingShot.id.startsWith('temp-') && !existingShot.id.startsWith('generating-')) {
        const { error: updateError } = await supabase
            ...
            .update({
                ...
                video_url: existingShot.video_url, // Preserve existing video URL when just replacing image
            })
      ...
      updateClientShotState(selectedScene.id, {
        ...
        isGenerating: false,
      }, existingShot?.id || `temp-error-${Date.now()}`); // Use existingShot.id or a temp one if it was new
    } finally {
      setIsLoading(false);
      if (event.target) {
        event.target.value = '';
  ...
  const handleGenerateShot = async () => {
    ...
    try {
      ...
      if (shotIdForUpdate) {
         const { error: updateError } = await supabase
            ...
            .update({
                ...shotBaseData,
                animation_prompt: existingShot?.animation_prompt || "",
                video_url: null, // New image, so clear video
            })
      ...
      await fetchData(); // Fetch data to revert to last good state or show error placeholder
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleRemoveShot = async () => {
    ...
    if (shotToRemove.id.startsWith('generating-') || shotToRemove.id.startsWith('temp-')) {
      updateClientShotState(selectedScene.id, { shots: [] } as any, shotToRemove.id); // Clear the temporary shot
      return;
    }
    setIsLoading(true);
    try {
      ...
      // Attempt to delete video from storage
  ...
  const handleAnimationPromptChange = async (sceneId: string, shotId: string, newPrompt: string) => {
    ...
    try {
        ...
        await fetchData(); // Re-fetch to ensure UI consistency if save failed
    }
  };
  ...
  const handleAnimateShot = async (sceneId: string, shotId: string) => {
    ...
    try {
        ...
        // Download the video file from itvFileUrl
        ...
        // Upload the downloaded video to Supabase Storage
        const videoFileName = `${shot.id}-animated-${Date.now()}.mp4`;
        const videoFilePath = `public/videos/${selectedScene.id}/${videoFileName}`; // Store in a 'videos' subfolder
        ...
        const { error: uploadError } = await supabase.storage
            ...
            .upload(videoFilePath, videoBlob, {
                ...
                upsert: true, // Overwrite if a file with the same name exists (e.g., re-animation)
            });
        ...
        // Update the shot in the database with the new video_url
        ...
        await fetchData(); // Refresh all data to get the latest state

    } catch (e: any) {
        console.error("Error during animation process:", e);
        setError(`Animation process failed: ${e.message}`);
        // Ensure isAnimating is reset on error by re-fetching or explicitly setting
        updateClientShotState(sceneId, { isAnimating: false, name: shot.name.replace(/^Animating: /,'').replace(/^Processing video.../, '') }, shotId); // Reset animation state on error
    } finally {
        setIsLoading(false);
        // Final check to ensure isAnimating is false if still true after everything.
  ...
  const sectionContainerClass = `p-4 rounded-lg shadow mt-4 ${backgrounds[PAGE_KEY] || backgrounds['global'] ? 'bg-gray-100/80 backdrop-blur-sm' : 'bg-gray-100'}`;
  ...
  const renderSeasons = () => (
    <div className={sectionContainerClass}>
      ...
      <button onClick={handleAddSeason} className={`${buttonClass} mb-2`} aria-label="Add new season" disabled={isLoading}>Add Season</button>
      {seasons.length === 0 && !isLoading && <p className="text-gray-500">No seasons yet. Click "Add Season" to start.</p>}
  ...
  const renderChapters = () => {
    ...
    return (
      <div className={sectionContainerClass}>
        ...
        <button onClick={handleAddChapter} className={`${buttonClass} mb-2`} aria-label={`Add chapter to ${selectedSeason.name}`} disabled={isLoading}>Add Chapter</button>
        {selectedSeason.chapters.length === 0 && !isLoading && <p className="text-gray-500">No chapters in this season yet.</p>}
  ...
  const renderEpisodes = () => {
    ...
    return (
      <div className={sectionContainerClass}>
        ...
        <button onClick={handleAddEpisode} className={`${buttonClass} mb-2`} aria-label={`Add episode to ${selectedChapter.name}`} disabled={isLoading}>Add Episode</button>
        {selectedChapter.episodes.length === 0 && !isLoading && <p className="text-gray-500">No episodes in this chapter yet.</p>}
  ...
  const renderScenes = () => {
    ...
    return (
      <div className={sectionContainerClass}>
        ...
        <button onClick={handleAddScene} className={`${buttonClass} mb-2`} aria-label={`Add scene to ${selectedEpisode.name}`} disabled={isLoading}>Add Scene</button>
        {selectedEpisode.scenes.length === 0 && !isLoading && <p className="text-gray-500">No scenes in this episode yet.</p>}
    ...
    return (
      <div className={sectionContainerClass}>
        <div className="flex justify-between items-center mb-3">
          <h2 className="text-xl font-semibold text-gray-700">Shot in {selectedScene.name}</h2>
          {shot && (
            <button
        ...
        <div className="flex space-x-2 mb-4">
            <label htmlFor={`shot-upload-${selectedScene.id}`} className={`${buttonClass} cursor-pointer inline-block ${isLoading || (shot && (shot.isGenerating || shot.isAnimating)) ? 'opacity-50 cursor-not-allowed' : ''}`}>
            ...
            <input
                ...
                disabled={isLoading || (shot && (shot.isGenerating || shot.isAnimating))}
            />
            <button
                ...
                disabled={isLoading || (shot && (shot.isGenerating || shot.isAnimating))}
            >
                {shot && shot.image_url && !shot.image_url.startsWith('https://via.placeholder.com') ? 'Generate New Image' : 'Generate Shot'}
            </button>
        </div>

        {!shot && !isLoading && <p className="text-gray-500 mt-2">No shot in this scene yet. Upload or generate one.</p>}
        ...
        {shot && (
          <div className={shotContainerClass}>
            ...
            <div className={shotMediaContainerClass}>
              {(shot.isAnimating || shot.isGenerating) && (
                <div className={shotOverlayClass}>
                  ...
                  <span>{shot.isAnimating ? 'Animating...' : shot.isGenerating ? 'Generating Image...' : 'Processing...'}</span>
                  {shot.name && (shot.name.startsWith("Generating:") || shot.name.startsWith("Animating:") || shot.name.startsWith("Uploading") || shot.name.startsWith("Processing video") ) && 
  ...
  return (
    <div className="flex flex-col min-h-screen bg-transparent">
        ...
        <main className="flex-grow container mx-auto px-4 py-8">
            <div className={`p-2 sm:p-6 rounded-lg shadow-inner min-h-[calc(100vh-12rem)] ${backgrounds[PAGE_KEY] || backgrounds['global'] ? 'bg-gray-50/80 backdrop-blur-sm' : 'bg-gray-50'}`}>
                ...
                <nav aria-label="Breadcrumb" className={`mb-6 p-3 rounded-md shadow-sm ${backgrounds[PAGE_KEY] || backgrounds['global'] ? 'bg-white/90' : 'bg-white'}`}>
                    <ol className="flex items-center space-x-2 text-sm flex-wrap">
                    <li><button onClick={() => handleSelectSeason(null)} className={`${breadcrumbButtonClass} ${!selectedSeasonId ? activeBreadcrumbClass : inactiveBreadcrumbClass}`} aria-current={!selectedSeasonId ? 'page' : undefined} disabled={isLoading || !!editingItem || !!editingDescriptionItem}>All Seasons</button></li>
